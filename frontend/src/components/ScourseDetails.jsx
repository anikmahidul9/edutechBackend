...
import {
  ...
  FaCheckCircle,
  FaLock,
  FaTimes, // FaTimes might not be needed if modal is removed
  FaUserGraduate, // FaUserGraduate might not be needed if modal is removed
} from "react-icons/fa";
...
const ScourseDetails = () => {
  ...
  const checkEnrollmentStatus = async () => {
    try {
      if (!currentUser || !course) return; // Ensure currentUser and course exist before querying
      const q = query(
        collection(db, "enrollments"),
        where("userId", "==", currentUser.uid),
        where("courseId", "==", course.id),
        where("status", "==", "paid") // Only consider paid enrollments
      );
  ...
  const handleEnroll = async () => { // Renamed from handleEnrollClick, new logic for payment initiation
    if (!isLoggedIn) {
      navigate("/student/login"); // Redirect to login if not logged in
      return;
    }
    if (isEnrolled) {
  ...
  const canAccessVideo = () => {
    return isEnrolled || course.price === "Free"; // Allow if enrolled or free
  };
